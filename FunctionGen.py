        ...
        for y in range(image.size[1]):
            for x in range(image.size[0]):
                ...
                for block in blocks.keys():
                    __ = ((pixel[0] - blocks[block][0]) ** 2 + (pixel[1] - blocks[block][1]) ** 2 + (
                                pixel[2] - blocks[block][2]) ** 2) ** 0.5
                    if __ < sim:
                        sim = __
                        sim_block = block
                file.write("setblock ~"+str(image.size[0]-x)+" ~"+str(image.size[1]-y)+" ~ "+sim_block+"\n")
                image.putpixel((x,y), blocks[sim_block])
                print(y*image.size[0]+x)
                if (y*image.size[0]+x)%1000==0:
                    file = open("fun"+str(int((y*image.size[0]+x)/1000))+".mcfunction","wt")
