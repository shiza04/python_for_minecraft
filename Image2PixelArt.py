...
def main():
    if len(sys.argv) == 2:
        ...
        pixelart = Image.new("RGB",(image.size[0]*16,image.size[1]*16))
            ...
            for x in range(image.size[0]):
                ...
                for block in blocks.keys():
                    ...
                    if __ < sim:
                        sim = __
                        sim_block = block
                _block = Image.open("blocks\\"+sim_block+".png")
                for _y in range(16):
                    for _x in range(16):
                        pixelart.putpixel((x*16+_x, y*16+_y),_block.getpixel((_x,_y)))
            print((y+1)/image.size[1],end="\r")
        pixelart.save("DIO"+sys.argv[1].split("\\")[-1])
    else:
